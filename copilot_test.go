...
func TestNew_WorkDirDefault(t *testing.T) {
	...
	llm, ok := d.(*LLM)
	if !ok {
		t.Fatalf("New returned unexpected type: %T", d)
	}
	if llm.workDir != "." {
...
func TestReload_UpdatesAllFields(t *testing.T) {
	...
	if err := llm.Reload(map[string]any{
		"work_dir":     "/new",
...
func TestReload_EmptyWorkDirKeepsOld(t *testing.T) {
	...
	if err := llm.Reload(map[string]any{"work_dir": ""}); err != nil {
		t.Fatalf("Reload failed: %v", err)
	}
	// readStringOpt falls back to fallback (".")  when value is empty,
	// not to the old value; check the actual behaviour.
	// readStringOpt falls back to fallback "." when value is empty (no-op assertion above)
